...
axios.get('http://localhost:3000/').then(({data: {prime, publicKey}}) => {
    ...
    axios.post('http://localhost:3000/session', {publicKey: clientPublicKey}).then(({data}) => {
        ...
        axios.get('http://localhost:3000/resource').then(({data: {txt}}) => {
            ...
            fs.writeFileSync(__dirname + '/decrypted.txt', decryptedTxt);
        });

        axios.get('http://localhost:3000/verified-resource').then(({data: {txt, ver}}) => {
            fs.writeFileSync(__dirname + '/file.txt', txt);
